<x-guest-layout>
    <div class="space-y-6" x-data="{ show: false }">
        <div class="text-center">
            ...
            <p class="mt-1 text-sm text-white/70">Log in to reserve a private movie room.</p>
        </div>

        <x-auth-session-status class="text-center text-sm text-emerald-200" :status="session('status')" />
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div>
                ...
                <div class="relative mt-1">
                    ...
                    >
                        <svg x-show="!show" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                        ...
                        <svg x-show="show" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5" style="display:none">
            ...
            <div class="flex items-center justify-between pt-1 text-sm">
                <label class="inline-flex items-center gap-2 text-white/70">
                    <input id="remember_me" type="checkbox" class="rounded border-white/20 bg-white/10 text-indigo-500 focus:ring-indigo-400" name="remember">
                    <span>{{ __('Remember me') }}</span>
                </label>

                @if (Route::has('password.request'))
                    <a class="text-indigo-200 hover:text-white" href="{{ route('password.request') }}">
                        {{ __('Forgot password?') }}
                    </a>
                @endif
            </div>
